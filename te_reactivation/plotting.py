...
1. Per-family footprint + background panels for ALL families
...
def plot_family_footprints(
    ...
    """
    Plot a footprint + background panel for EACH TE family.

    Each family gets its own PNG with two subplots:
      Top: positional footprint (sense/antisense across TE body) if BAM profiles available
      Bottom: TE body vs flank comparison (bar chart per locus, sorted)

    Saves to save_dir/footprint_<family>.png
    """
    ...
    # Sort families by P(reactivated) descending
    order = np.argsort(-z)
    ...
    for f_idx in order:
        ...
        # ---- Panel 1: Positional footprint (if available) ----
        if has_profiles:
            ax = axes[row]
            ...
            ax.fill_between(bin_centers, sense_prof, alpha=0.5, color="#2166ac", label="Sense")
            ax.fill_between(bin_centers, -antisense_prof, alpha=0.5, color="#b2182b", label="Antisense")
            ...
            ax.set_title("Positional Footprint", fontsize=11)
            ax.legend(fontsize=8, loc="upper right")
            row += 1

        # ---- Panel 2: TE body vs flank background per locus ----
        ax = axes[row]

        # Compute per-locus rates (reads per kb) for TE body
        te_sense_rate = sense / (lengths_kb + 1e-6)
        te_antisense_rate = antisense / (lengths_kb + 1e-6)

        # Sort by TE body total rate descending
        ...
        if has_flanks:
            ...
            # Flank line overlay (shows local background level)
            ...
            # Compute fold-change over flanks
        ...
        ax.set_title("TE Body vs Flank Background", fontsize=11)
...
def plot_sense_antisense(
    ...
    """Sense vs antisense mean counts per family, colored by reactivation."""
    z = results["z_posterior"]
    names = results["family_names"]
